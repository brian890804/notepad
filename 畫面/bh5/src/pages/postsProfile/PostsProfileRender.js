...
const PostsProfilePage = ({
  ...
  const [moreSettingList, setMoreSettingList] = useState(() => [
    ...
    },
    {
      ...
      onClick: () => postCardAttentionEvent(postProfile.profile),
    },
    {
      text: "取消",
  ...
  useLayoutEffect(() => {
    if (router) postGetProfile(contentGuide[nowGuide].key);
    return () => {
      cleanProfile();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  function scrollEvent() {
    scrollBottomCallEvent((scrollColdEnd) => {
      if (!postProfile.isDone) {
        getPostListAction(scrollColdEnd);
      }
    });
  }
  ...
  function onSubscribe(type) {
    ...
    if (type) {
      postCardScribeMediaEvent(postProfile.profile, type);
    }
  }
  return (
    <PostsProfilePageElement>
      ...
      <PostsProfileContent transIndex={nowGuide}>
        <div className="post_content_guide">
          {contentGuide.map((item, index) => (
            <div
              key={item.key}
              value={index}
              className={`post_content_guide_item ${
                nowGuide === index && "active"
              }`}
        ...
        <section className="post_main_container">
          {postProfile?.postList.map((data, index) => {
            return (
