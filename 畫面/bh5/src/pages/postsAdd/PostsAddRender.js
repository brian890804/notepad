...
import girIcon from "../../assets/post/girl_icon.png";
...
const PostsAddPage = ({
  ...
  const [monthMoney, setMonthMoney] = useState(user.monthly_price || null);
  const [yearMoney, setYearMoney] = useState(user.yearly_price || null);
  ...
  function addPostCard(updataImg = false) {
    ...
    if (nowSelectTags.length > 0) {
      formData.append(
        "tag",
        nowSelectTags.map((data) => data.id)
      );
    }
    ...
    axiosRequest.post(postAddDynamicUrl, formData).then((data) => {
      if (user.rank === userRank[0] && user.is_creation === 0) {
        callToast("上传完成请等待审核");
      } else {
        callToast("发布成功");
      }
  ...
  return (
    <PostsAddPageElement uploadMount={postFileArray.length}>
      <CSSTransition
        timeout={200}
      ...
      {user.is_creation ? (
        ...
        <div className="original_owner">
          <img src={girIcon} alt="girlIcon" className="original_owner_icon" />
...
export const PostsAddPageElement = styled.div`
  /*  */
    ...
    &_body {
      ...
      &_effect {
        &_area {
          ...
          @media (max-width: 899px) {
            justify-content: ${({ uploadMount }) =>
              uploadMount >= 1 ? "flex-end" : " space-between"};
            flex-direction: ${({ uploadMount }) =>
              uploadMount >= 1 ? "column" : "row"};
          }
        }
        &_tags {
          cursor: pointer;
